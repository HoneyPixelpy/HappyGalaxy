...
class RabbitMQAnalyticsBD:
    ...
    @logger.catch
    def process_shop_purchase(self, summary: AnalyticsSummary, event_data: Dict):
    ...
    def add_reward(self, user_id: int, quest: Quests) -> float:
        """
        Расчет награды
        """
    ...
    def process_quest_action(self, summary: AnalyticsSummary, event_data: Dict):
        """
        fast_success
        attempt
        success
        ...
        quest_stats, _ = QuestStats.objects.get_or_create(quest=quest, summary=summary)
    ...
    def process_game_action(self, summary: AnalyticsSummary, event_data: Dict):
        user_id = event_data["user_id"]
        win_starcoins = event_data["win_starcoins"]
        game = event_data["game"]

        quest_stats, _ = GamesStats.objects.get_or_create(summary=summary)
    ...
    def process_user_action(self, summary: AnalyticsSummary, event_data: Dict):
        """
...
class RabbitMQAnalyticsConsumer(RabbitMQAnalyticsBD):
    ...
    def connect(self) -> bool:
        ...
        while attempt < self.max_reconnect_attempts and self.running:
            try:
                logger.info(
                    f"🔗 Connecting to RabbitMQ (attempt {attempt + 1}/{self.max_reconnect_attempts})..."
                )
    ...
    def _on_message_callback(self, channel, method, properties, body):
        ...
        try:
            ...
            logger.debug(
                f"📨 Received message from {queue_name}: {event_data.get('event_type', 'unknown')}"
            )

            success = self.process_event_with_ack(
                channel, method, queue_name, event_data
            )
            ...
            if success:
                ...
                logger.warning(
                    f"⚠️ Failed to process {queue_name} after {processing_time:.2f}s"
                )
        ...
        try:
            ...
            with transaction.atomic():
                ...
                if processing_time > max_processing_time:
                    logger.warning(
                        f"⚠️ Slow processing: {processing_time:.2f}s for {queue_name}"
                    )
