...
class Users(BaseModel):
    ...
    vk_id = models.BigIntegerField(
        unique=True, null=True, blank=True, verbose_name="VK ID"
    )  # BigIntegerField

    tg_first_name = models.CharField(
        max_length=255, blank=True, null=True, verbose_name="TG Имя"
    )
    tg_last_name = models.CharField(
        max_length=255, blank=True, null=True, verbose_name="TG Фамилия"
    )
    tg_username = models.CharField(
        max_length=255, blank=True, null=True, verbose_name="TG Юзернейм"
    )

    referral_user_id = models.BigIntegerField(
        blank=True, null=True, verbose_name="Telegram ID Реферала"
    )  # BigIntegerField

    authorised = models.BooleanField(default=False, verbose_name="Авторизован")
    _authorised_at = models.DateTimeField(
        null=True,
        blank=True,
        db_column="authorised_at",
    ...
    _age = models.DateTimeField(
        blank=True, null=True, db_column="age", verbose_name="Дата рождения"
    )

    name = models.CharField(max_length=255, blank=True, null=True, verbose_name="Имя")
    supername = models.CharField(
    ...
    def authorised_at(self):
        ...
        return timezone.localtime(
            self._authorised_at, timezone=ZoneInfo("Europe/Moscow")
        )
    ...
    def age(self):
        ...
        if self._age is None:
            return None
        return timezone.localtime(self._age, timezone=ZoneInfo("Europe/Moscow"))
    ...
    def starcoins(self):
        return (
            int(self._starcoins)
            if round(self._starcoins, 4) == int(self._starcoins)
            else round(self._starcoins, 4)
        )
    ...
    def starcoins(self, value):
        logger.info(
            "Change Balance: UserID:{0} |Old Balance:{1} |New Balance:{2} |Edit:{3}".format(
                self.user_id, self._starcoins, value, self._starcoins - value
            )
        )
    ...
    def send_rang_notification(
        self: "Users", current_rang: "Rangs", previous_rang: "Rangs"
    ) -> None:
        """Уведомляем пользователя о повышении"""
        new_quests = (
            Quests.objects.filter(min_rang_level=previous_rang.level, active=True)
            .filter(
        ...
        RangService().send_rang_notification(
            self, current_rang, previous_rang, bool(new_quests)
        )
...
class Family_Ties(BaseModel):
    ...
    to_user = models.ForeignKey(
        Users, on_delete=models.CASCADE, related_name="to_relationships"
    )
...
class Purchases(BaseModel):
    ...
    _purchase_date = models.DateTimeField(
        auto_now_add=True, db_column="purchase_date", verbose_name="Дата покупки"
    )
    ...
    _completed_at = models.DateTimeField(
        null=True, blank=True, db_column="completed_at", verbose_name="Дата выдачи"
    )
    ...
    def purchase_date(self):
        ...
        if self._purchase_date is None:
            return None
        return timezone.localtime(
            self._purchase_date, timezone=ZoneInfo("Europe/Moscow")
        )
    ...
    def completed_at(self):
        ...
        if self._completed_at is None:
            return None
        return timezone.localtime(
            self._completed_at, timezone=ZoneInfo("Europe/Moscow")
        )
    ...
    def cost(self):
        return (
            int(self._cost)
            if round(self._cost, 4) == int(self._cost)
            else round(self._cost, 4)
        )
...
class Pikmi_Shop(BaseModel):
    title = models.CharField(
        blank=True, null=True, max_length=255, verbose_name="Заголовок"
    )
    description = models.TextField(blank=True, null=True, verbose_name="Описание")

    _price = models.FloatField(
        null=True, blank=True, db_column="price", verbose_name="Цена"
    )
    ...
    delivery_instructions = models.TextField(
        blank=True, null=True, verbose_name="Инструкция к выдаче"
    )
    ...
    def price(self):
        return (
            int(self._price)
            if round(self._price, 4) == int(self._price)
            else round(self._price, 4)
        )
...
class Sigma_Boosts(BaseModel):
    user = models.OneToOneField(
        Users, on_delete=models.CASCADE, related_name="boosts", verbose_name="Игрок"
    )
    # Улучшения
    income_level = models.IntegerField(
        default=0, verbose_name="lvl За клик"
    )  # Уровень дохода за клик (0-19)
    energy_capacity_level = models.IntegerField(
        default=0, verbose_name="lvl Макс. энергии"
    )  # Уровень запаса энергии (0-7)
    recovery_level = models.IntegerField(
        default=0, verbose_name="lvl Время восстановления"
    )  # Скорость восстановления (0-2) РАБОТАЕМ С МИНУТАМИ
    passive_income_level = models.IntegerField(
        default=0, verbose_name="lvl Пассивный заработок"
    )  # Пассивный доход (0-3)
    _last_passive_claim = models.DateTimeField(
        default=timezone.now, db_column="last_passive_claim"
    )  # Последний сбор пассивного дохода
    ...
    def last_passive_claim(self):
        ...
        if self._last_passive_claim is None:
            return None
        return timezone.localtime(
            self._last_passive_claim, timezone=ZoneInfo("Europe/Moscow")
        )
    ...
    def save(self, *args, **kwargs):
        ...
        if (
            ...
            and self._last_passive_claim.tzinfo != datetime_timezone.utc
        ):
            self._last_passive_claim = self._last_passive_claim.astimezone(
...
class Lumberjack_Game(BaseModel):
    user = models.ForeignKey(
        Users, on_delete=models.CASCADE, related_name="games", verbose_name="Игрок"
    )
    ...
    _last_energy_update = models.DateTimeField(
        default=timezone.now, db_column="last_energy_update"
    )  # Последнее обновление энергии
    total_clicks = models.BigIntegerField(
        default=0, verbose_name="Всего кликов"
    )  # BigIntegerField
    ...
    def last_energy_update(self):
        ...
        if self._last_energy_update is None:
            return None
        return timezone.localtime(
            self._last_energy_update, timezone=ZoneInfo("Europe/Moscow")
        )
    ...
    def save(self, *args, **kwargs):
        ...
        if (
            ...
            and self._last_energy_update.tzinfo != datetime_timezone.utc
        ):
            self._last_energy_update = self._last_energy_update.astimezone(
                datetime_timezone.utc
            )
        super().save(*args, **kwargs)
    ...
    def total_currency(self):
        return (
            int(self._total_currency)
            if round(self._total_currency, 4) == int(self._total_currency)
            else round(self._total_currency, 4)
        )
...
class GeoHunter(BaseModel):
    user = models.ForeignKey(
        Users, on_delete=models.CASCADE, related_name="geo_hunter", verbose_name="Игрок"
    )
    game_date = models.DateTimeField(
        auto_now_add=True, verbose_name="Дата создания игры"
    )  # NOTE ненужен
    current_energy = models.IntegerField(default=0, verbose_name="Текущая энергия")
    max_energy = models.IntegerField(default=0, verbose_name="Макс. энергия")
    _last_energy_update = models.DateTimeField(
    ...
    total_true = models.BigIntegerField(
        default=0, verbose_name="Общее количество правильных ответов"
    )  # BigIntegerField
    total_false = models.BigIntegerField(
        default=0, verbose_name="Общее количество не правильных ответов"
    )  # BigIntegerField
    _total_currency = models.FloatField(default=0.0, verbose_name="Всего заработал")
...
class Work_Keys(BaseModel):
    from_user = models.ForeignKey(
        Users, on_delete=models.CASCADE, related_name="work_key", blank=True, null=True
    )
...
class AddStarcoinsBonus(BaseModel):
    ...
    def value(self):
        return (
            int(self._value)
            if round(self._value, 4) == int(self._value)
            else round(self._value, 4)
        )
...
class ClickScaleBonus(BaseModel):
    ...
    def duration_hours(self):
        return (
            int(self._duration_hours)
            if round(self._duration_hours, 4) == int(self._duration_hours)
            else round(self._duration_hours, 4)
        )
...
class Bonuses(BaseModel):
    ...
    type_bonus = models.CharField(
        max_length=255, choices=BONUS_TYPES, blank=True, null=True, verbose_name="Тип"
    )

    active = models.BooleanField(default=True, verbose_name="Статус")

    _expires_at = models.DateTimeField(
        db_column="expires_at", blank=True, null=True, verbose_name="Дата истечения"
    )
    ...
    def expires_at(self):
        ...
        if self._expires_at is None:
            return None
        return timezone.localtime(self._expires_at, timezone=ZoneInfo("Europe/Moscow"))
...
class UseBonuses(BaseModel):
    user = models.ForeignKey(Users, on_delete=models.CASCADE, related_name="user_bonus")
    bonus = models.ForeignKey(
        Bonuses, on_delete=models.CASCADE, related_name="use_bonus"
    )
...
class SubscribeQuest(BaseModel):
    ...
    chat_id_name = models.CharField(
        max_length=255, null=True, verbose_name="chat_id или chat_name"
    )
    _reward_starcoins = models.FloatField(
        db_column="reward_starcoins", verbose_name="Вознаграждение"
    )
    type = models.CharField(
        max_length=255, null=True, choices=types, verbose_name="Тип"
    )  # tg ; vk ; wa
    ...
    def reward_starcoins(self):
        return (
            int(self._reward_starcoins)
            if round(self._reward_starcoins, 4) == int(self._reward_starcoins)
            else round(self._reward_starcoins, 4)
        )
...
class IdeaQuests(BaseModel):
    ...
    call_action = models.TextField(
        blank=True, null=True, verbose_name="Требования к контенту"
    )
    content = models.CharField(
        max_length=255, null=True, choices=contents, verbose_name="Тип контента"
    )
    ...
    type = models.CharField(
        max_length=255, null=True, default="galactic_idea"
    )  # galactic_idea ; descr_happy ; show_happy
...
class DailyQuests(BaseModel):
    ...
    scale_type = models.CharField(
        max_length=255, null=True, choices=scale_types, verbose_name="Тип скейла"
    )  # null ; x_count_use
    type = models.CharField(
        max_length=255,
        null=True,
...
class Quests(BaseModel):
    ...
    min_rang_level = models.IntegerField(
        default=1, verbose_name="Минимальный уровень ранга"
    )
    max_rang_level = models.IntegerField(
        default=999999, verbose_name="Максимальный уровень ранга"
    )
    success_admin = models.BooleanField(default=True, verbose_name="Проверка админом")

    _expires_at = models.DateTimeField(
        db_column="expires_at", blank=True, null=True, verbose_name="Дата просрочки"
    )
...
class UseQuests(BaseModel):
    user = models.ForeignKey(
        Users, on_delete=models.CASCADE, related_name="user_quests"
    )
    quest = models.ForeignKey(
        Quests, on_delete=models.CASCADE, related_name="use_quest"
    )
    count_use = models.IntegerField(
        default=1, null=True, verbose_name="Использован раз"
    )
...
class QuestModerationAttempt(BaseModel):
    use_quest = models.ForeignKey(
        UseQuests, on_delete=models.CASCADE, related_name="moderation_attempts"
    )
...
class ReferralConnections(BaseModel):
    referer = models.ForeignKey(
        Users, on_delete=models.CASCADE, related_name="user_referer"
    )
    referal = models.ForeignKey(
        Users, on_delete=models.CASCADE, related_name="user_referal"
    )
    ...
    activate = models.BooleanField(
        default=True, verbose_name="Был ли выдан бонус за приглашение"
    )
...
class Rangs(BaseModel):
    ...
    def __str__(self):
        return (
            f"{self.level} {self.all_starcoins} {self._role} {self.emoji} {self.name}"
        )
...
class Promocodes(BaseModel):
    ...
    used_quantity = models.IntegerField(
        default=0, verbose_name="Количество использованных"
    )
    ...
    _expires_at = models.DateTimeField(
        blank=True, null=True, db_column="expires_at", verbose_name="Действителен до"
    )
...
class UsePromocodes(BaseModel):
    user = models.ForeignKey(
        Users, on_delete=models.CASCADE, related_name="user_promocode"
    )
    promocode = models.ForeignKey(
        Promocodes, on_delete=models.CASCADE, related_name="use_promocode"
    )
...
class ManagementLinks(BaseModel):
    LINK_TYPES = (("authorised_start", "регистрация-старт"),)

    type_link = models.CharField(
        max_length=255, choices=LINK_TYPES, blank=True, verbose_name="Тип ссылки"
    )
...
class UseManagementLinks(BaseModel):
    user = models.ForeignKey(
        Users, on_delete=models.CASCADE, related_name="user_management"
    )
    management_link = models.ForeignKey(
        ManagementLinks, on_delete=models.CASCADE, related_name="management_link"
    )
...
class InteractiveGames(BaseModel):
    ...
    user = models.ForeignKey(
        Users, on_delete=models.CASCADE, related_name="created_interactive_games"
    )
    ...
    reward_type = models.CharField(
        max_length=255,
        default="to_each_winner",
        choices=REWARD_TYPES,
        blank=True,
        verbose_name="Способ распределения вознаграждения",
    )

    min_rang = models.IntegerField(default=0, verbose_name="Минимальный уровень ранга")
    max_rang = models.IntegerField(
        default=999999, verbose_name="Максимальный уровень ранга"
    )

    min_players = models.IntegerField(
        default=0, verbose_name="Минимальное количество игроков"
    )
    max_players = models.IntegerField(
        default=999999, verbose_name="Максимальное количество игроков"
    )

    type_game = models.CharField(
        max_length=255, choices=GAME_TYPES, blank=True, verbose_name="Тип игры"
    )
    ...
    _start_invite_at = models.DateTimeField(
        null=True, verbose_name="Время начала приглашения"
    )
    _start_game_at = models.DateTimeField(null=True, verbose_name="Время начала игры")
    _ended_game_at = models.DateTimeField(
        null=True, verbose_name="Время завершения игры"
    )
    ...
    def start_invite_at(self):
        ...
        if self._start_invite_at is None:
            return None
        return timezone.localtime(
            self._start_invite_at, timezone=ZoneInfo("Europe/Moscow")
        )
    ...
    def start_game_at(self):
        ...
        if self._start_game_at is None:
            return None
        return timezone.localtime(
            self._start_game_at, timezone=ZoneInfo("Europe/Moscow")
        )
    ...
    def ended_game_at(self):
        ...
        if self._ended_game_at is None:
            return None
        return timezone.localtime(
            self._ended_game_at, timezone=ZoneInfo("Europe/Moscow")
        )
...
class GameData(BaseModel):
    ...
    user = models.ForeignKey(
        Users, on_delete=models.CASCADE, related_name="game_results"
    )
    game = models.ForeignKey(
        InteractiveGames, on_delete=models.CASCADE, related_name="game_results"
    )
...
class CallbackAction(BaseUserAction):
    summary = models.ForeignKey(
        AnalyticsSummary, on_delete=models.CASCADE, related_name="callback_action"
    )
...
class DailyUserStats(BaseModel):
    summary = models.ForeignKey(
        AnalyticsSummary, on_delete=models.CASCADE, related_name="user_stats"
    )
    user_id = models.BigIntegerField()
    ...
    peak_activity_hour = models.IntegerField(
        null=True
    )  # Час с максимальной активностью (0-23)
...
class DailyButtonStats(BaseModel):
    summary = models.ForeignKey(
        AnalyticsSummary, on_delete=models.CASCADE, related_name="button_stats"
    )
    ...
    avg_time_to_click = models.DurationField(
        null=True
    )  # Среднее время до клика с начала сессии
...
class ShopStats(BaseModel):
    ...
    unique_buyers = models.JSONField(
        default=list, verbose_name="Уникальные пользователи"
...
class QuestStats(BaseModel):
    ...
    total_rewards = models.FloatField(
        default=0.0, verbose_name="Всего наград"
    )  # NOTE переименовать в rewards
