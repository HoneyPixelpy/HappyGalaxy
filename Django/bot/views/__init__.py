...
class SigmaBoostsViewSet(ViewSet, AbstractSigmaBoosts):
    ...
    def upgrade_boost(self, request):
        ...
        SigmaBoostsViewMethods.upgrade_boost(
            user, user_boosts, jack_game, geo_hunter, name
        )
...
class LumberjackGameViewSet(ViewSet, AbstractGame, AbstractLumberjackGame):
    ...
    def process_click(self, request):
        ...
        LumberjackGameViewMethods.process_click(
            user, game_user, game_user_two, boosts_user, energy_in_click, row, col
        )
...
class GeoHunterGameViewSet(ViewSet, AbstractGame):
    ...
    def process_click(self, request):
        ...
        GeoHunterViewMethods.process_click(
            user, game_user, game_user_two, boosts_user, energy_in_click, user_choice
        )
...
class RewardViewSet(ViewSet):
    ...
    def list(self, request):
        ...
        """
        return Response(reward_data.model_dump(), status=status.HTTP_200_OK)
...
class UserViewSet(ViewSet):
    ...
    def check_phone(self, request):
        ...
        return Response(serializer.data if user else None, status=status.HTTP_200_OK)
    ...
    def complete_registration(self, request, pk=None):
        ...
        return UserMethods.complete_registration(user, state_data, rollback)
...
class PurchasesViewSet(ViewSet):
    ...
    # PATCH /api/v1/purchases/add_message_id/
    @action(detail=False, methods=["patch"])  # , url_path='confirm-purchase'
    @queue_request
    def add_message_id(self, request):
        ...
        PurchasesMethods.edit_message_id(purchase, message_id)

    # PATCH /api/v1/purchases/cancel_buy/
    @action(detail=False, methods=["patch"])  # , url_path='confirm-purchase'
    @queue_request
    def cancel_buy(self, request):
        """
        Отменяем покупку
        """
        purchases_id = QueryData.check_params(request, "purchases_id")
...
class WorkKeysViewSet(ViewSet):
    ...
    def check_by_key(self, request):
        ...
        return Response(not bool(key.from_user), status=status.HTTP_200_OK)
...
class BonusesViewSet(ViewSet):
    ...
    def create(self, request):
        ...
        if type_bonus == "add_starcoins":
            ...
            bonus_obj = BonusesMethods.create_add_starcoins(
                value=value, max_quantity=max_quantity
            )
            ...
            bonus_obj = BonusesMethods.create_click_scale(
                value=value, duration_hours=duration
            )
        elif type_bonus == "energy_renewal":
            duration = QueryData.check_params(request, "duration_hours")

            bonus_obj = BonusesMethods.create_energy_renewal(duration_hours=duration)
        else:
            return Response(
                {"error": "Invalid bonus type"}, status=status.HTTP_400_BAD_REQUEST
            )

        return BonusesMethods.create(
            type_bonus=type_bonus, expires_at=expires_at, bonus_obj=bonus_obj
        )
    ...
    def claim_bonus(self, request):
        ...
        if not bonus.active:
            return Response({"text": "not_active"}, status=status.HTTP_200_OK)
        ...
        if bonus.type_bonus == "add_starcoins":
            ...
            return UseBonusesMethods.activate_energy_renewal(
                bonus=bonus, lumberjack=lumberjack, geohunter=geohunter, now=now
            )
        else:
...
class QuestsViewSet(ViewSet):
    ...
    def get_info(self, request):
        ...
        return QuestMethods.get_info(user=user, quest=quest)
...
class InteractiveGameViewSet(ViewSet):
    ...
    def retrieve(self, request, pk=None):
        ...
        return Response(serializer_game.data, status=status.HTTP_200_OK)
