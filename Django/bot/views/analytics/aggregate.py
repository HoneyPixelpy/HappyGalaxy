...
class AggregationData:
    ...
    def process_data(
        ...
        for summary in aggregat_data:
            ...
            logger.info(
                f"  ✓ Обработано: {len(messages)} сообщений, {len(callbacks)} колбэков"
            )
    ...
    def _aggregate_button_stats(
        self, summary: AnalyticsSummary, callbacks: List[CallbackAction]
    ) -> None:
        """
        ...
        button_data = defaultdict(
            lambda: {"clicks": [], "users": set(), "click_times": []}
        )
        ...
        for (button_text, button_data_str), data in button_data.items():
            ...
            user_retention_rate = (
                (repeat_users / unique_users * 100) if unique_users > 0 else 0
            )
    ...
    def _update_summary_stats(self, summary: AnalyticsSummary) -> None:
        """
        ...
        logger.info(
            f"  Обновлена статистика дня: {total_users} пользователей, "
...
class ClearData:
    ...
    @transaction.atomic
    def delete_work_data(self, summary_id: List[int]) -> None:
        """
        ...
        logger.debug(
            f"🗑️ Удалено: {messages_deleted[0]} сообщений, {callbacks_deleted[0]} колбэков"
        )


class BackupData:

    def create_backup(self) -> Response:
        """
        ...
        dump_content = self._subprocess_pg_dump(
            db_host, db_port, db_user, db_password, db_name
        )

        response = HttpResponse(
            dump_content, content_type="application/sql", status=status.HTTP_200_OK
        )
    ...
    def _subprocess_pg_dump(
        ...
        cmd = [
            ...
            db_name,
            "-t",
            "user_message_actions",  # ваша таблица MessageAction
            "-t",
            "user_callback_actions",  # ваша таблица CallbackAction
...
class AdvancedStatsAggregator(AggregationData, BackupData, ClearData):
    ...
    def get_old_summaries(self, cutoff_date: datetime) -> List[AnalyticsSummary]:
        """
        Вытаскиваем связанные AnalyticsSummary на каждый день
        старше 7 дней без user_stats или button_stats
        """
        return (
            AnalyticsSummary.objects.filter(date__lt=cutoff_date)
            .exclude(
    ...
    def get_data(self, summary_id: List[int]) -> List[Dict[str, Any]]:
        """
        Вытаскиваем интересующие данные
        """
        return AnalyticsSummary.objects.filter(id__in=summary_id).prefetch_related(
            Prefetch(
                "message_actions",
    ...
    @logger.catch
    def aggregate_stats(self, cutoff_date: datetime) -> Response:
        """
        ...
        if not old_summaries:
            return Response(
                data={"message": "Нет статистики за последние 7 дней"},
        ...
        if not aggregat_data:
            return Response(
                data={"message": "Нет данных для агрегации"},
                status=status.HTTP_404_NOT_FOUND,
            )
