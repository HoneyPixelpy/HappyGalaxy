...
from ..models import ManagementLinks, UseManagementLinks, UsePromocodes, Users, Promocodes
...
class PromocodesMethods(CodesAbstract):
    ...
    def create(
        ...
        if UsePromocodes.objects.filter(user=user, promocode=promocode).first():
            raise RaisesResponse(
                data={'error': 'use_promocodes'},
                status=status.HTTP_200_OK
            )

        if promocode.used_quantity >= promocode.all_quantity:
            raise RaisesResponse(
                data={'error': 'all_quantity'},
                status=status.HTTP_200_OK
            )

        if promocode.expires_at and promocode.expires_at < datetime.now(pytz.timezone('Europe/Moscow')):
            raise RaisesResponse(
                data={'error': 'expires_at'},
                status=status.HTTP_200_OK
            )
        ...
        if promocode.type_promo == 'starcoins':
            ...
            raise RaisesResponse(
                data={'error': 'Type Promo not found'},
        ...
        UsePromocodes.objects.create(
            user=user, 
            promocode=promocode
        )
        
        raise RaisesResponse(
            data={'data': serializer.data, 'type': _type},
            status=status.HTTP_200_OK
            )
...
class ManagementLinksMethods(CodesAbstract):
    ...
    def create(
        cls,
        user: Users,
        management_link: ManagementLinks
        ) -> Response:
        if UseManagementLinks.objects.filter(user=user, management_link=management_link).first():
            raise RaisesResponse(
                data={'error': 'use_promocodes'},
                status=status.HTTP_200_OK
            )

        UseManagementLinks.objects.create(
            user=user, 
