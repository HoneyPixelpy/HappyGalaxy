...
from ..models import Family_Ties, Lumberjack_Game, Quests, Rangs, ReferralConnections, Sigma_Boosts, SubscribeQuest, UseQuests, Users, Work_Keys
...
class UserMethods:
    ...
    def banned(cls) -> Response:
        ...
        banned_ids = list(Users.objects.filter(ban=True).values_list('user_id', flat=True)) # .all()
        raise RaisesResponse(
            data=banned_ids, 
    ...
    def complete_registration(
        ...
        if state_data["role"] in ["worker", "manager"] and not rollback:
            ...
            if game_user and not game_user.max_energy:
                game_user.max_energy = boosts_data.energy_capacity_level.value_by_level(0)
                game_user.current_energy = boosts_data.energy_capacity_level.value_by_level(0)
                game_user.save()
        ...
        user.authorised = True if state_data.get("authorised") is None else state_data.get("authorised")
        user.authorised_at = timezone.now() if not state_data.get("authorised_at") else state_data.get("authorised_at")
    ...
    def get_all_vk_users(
        ...
        serializer = UserSerializer(users_with_vk, many=True)  # Обратите внимание на many=True
        raise RaisesResponse(
    ...
    def back_vk_id(
        ...
        quest = Quests.objects.filter(
            ...
            object_id__in=subscribe_quest_ids
        ).first()
        ...
        logger.info(
            f"Сняли за отпуску ВК {quest.quest_data.reward_starcoins} у {user}"
        )
    ...
    def active_users(
        ...
        queryset = Users.objects.all().filter(
            active=True
            )
        for user in queryset:
            level = user.get_current_rang().level
            logger.debug(f"Пользователь {user} активен {level} -> {min_rang <= level <= max_rang}")

        serializer = UserSerializer(
            [user for user in queryset if min_rang <= user.get_current_rang().level <= max_rang], 
            many=True
            )
        raise RaisesResponse(
            data=serializer.data, 
...
class FamilyTiesMethods:
    ...
    def _create_family_tie(
        ...
        for game in [parent_game, child_game]:
            if game and not game.max_energy:
                ...
                game.current_energy = boosts_data.energy_capacity_level.value_by_level(0)
                game.save()
        
        return Family_Ties.objects.create(
            from_user=from_user,
            to_user=to_user
        )
    @classmethod
    ...
    def get_family(
        ...
        ties = Family_Ties.objects.filter(
    ...
    def get_target_ties(
        ...
        tie = Family_Ties.objects.filter(
            Q(from_user=parent, to_user=user) | 
            Q(to_user=parent, from_user=user)
        ).first()
        if tie:
            serializer = FamilyTiesSerializer(tie)
            raise RaisesResponse(
...
class RangsMethods:
    ...
    def role(
        ...
        rangs = Rangs.objects.filter(
            Q(_role=role) | Q(_role__isnull=True)
            ).order_by('all_starcoins').all()
