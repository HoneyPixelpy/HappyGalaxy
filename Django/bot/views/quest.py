...
from ..serializers import (
    DailyQuestsSerializer,
    IdeaQuestsSerializer,
    QuestsSerializer,
    SubscribeQuestSerializer,
    UserSerializer,
)
...
class QuestMethods:
    @classmethod
    def all(cls) -> List[Quests]:
        return Quests.objects.all()

    @classmethod
    def get(cls, pk: int) -> Quests:
        try:
            if pk:
    ...
    @classmethod
    def get_info(cls, user: Users, quest: Quests) -> Response:
            ...
            if quest.type_quest in ["idea", "daily"]:
                ...
                if count_use:
                    attempt = QuestModerationAttempt.objects.filter(
                        use_quest=user_quests, moderation_status="pending"
                    ).all()
                    if attempt:
                ...
                raise RaisesResponse(
                    data="Квест уже был использован", status=status.HTTP_404_NOT_FOUND
                )
        ...
        try:  # NOTE это для того чтобы возвращать правильное вознаграждение для "x_count_use"
            if quest.quest_data.scale_type == "x_count_use":
                if user_quests and quest.type_quest == "daily":
                    ...
                    serializer["quest_data"]["_reward_starcoins"] = (
                        quest.quest_data.reward_starcoins * result_count_use
                    )
        ...
        raise RaisesResponse(data=serializer, status=status.HTTP_200_OK)

    @classmethod
    def active(cls, user: Users, rang: Rangs) -> Response:
        quests = []

        # Сначала получаем все подходящие квесты
        filtered_quests = (
            Quests.objects.filter(
                active=True,
                min_rang_level__lte=rang.level,
                max_rang_level__gte=rang.level,
            )
            .filter(Q(role=user._role) | Q(role__isnull=True))
        ...
        sorted_quests = sorted(
            ...
            key=lambda x: (
                ...
                (
                    -getattr(getattr(x, "quest_data", None), "reward_starcoins", 0)
                    if hasattr(x, "quest_data") and x.quest_data
                    else 0
                ),
            ),
        )
...
class UseQuestMethods:
    @classmethod
    def get(cls, user: Users, quest: Quests) -> Response:
        try:
            return UseQuests.objects.get(user=user, quest=quest)
        except UseQuests.DoesNotExist:
            raise RaisesResponse(
                data={"error": "UseQuest not found"}, status=status.HTTP_404_NOT_FOUND
            )

    @classmethod
    def get_quests(cls, user: Users, quest: Quests) -> Response:
        user_quests = UseQuests.objects.filter(user=user, quest=quest).first()
        ...
        raise RaisesResponse(data=bool(user_quests), status=status.HTTP_200_OK)

    @classmethod
    def create(cls, user: Users, quest: Quests) -> Response:
            ...
            if quest.type_quest == "idea":
                ...
                if count_use:
                    if count_use > use_quest_obj.count_use:
                        ...
                        raise RaisesResponse(
                            data={
                                "error": "This quest is already assigned to the user"
                            },
                            status=status.HTTP_400_BAD_REQUEST,
                        )
            else:
    ...
    @classmethod
    def create_idea_daily(cls, user: Users, quest: Quests) -> Response:
        use_quest_obj = UseQuests.objects.filter(user=user, quest=quest).first()
        ...
        if not quest.success_admin:
            if (
                quest.type_quest == "daily"
                and quest.quest_data.scale_type == "x_count_use"
            ):
                add_starcoins = (
                    quest.quest_data.reward_starcoins * use_quest_obj.count_use
                )
            else:
            ...
            QuestModerationAttempt.objects.create(
                use_quest=use_quest_obj, attempt_number=use_quest_obj.count_use
            )
            result = True

        # NOTE число будет означать что за квест сразу расчитались и нужно уведомить пользователя об выигрыше
        raise RaisesResponse(data=result, status=status.HTTP_201_CREATED)

    @classmethod
    def sub_tg_chat(cls, quest: Quests) -> Response:
        users = [use_quest.user for use_quest in UseQuests.objects.filter(quest=quest)]
    ...
    @classmethod
    def sub_vk_chat(cls, quest: Quests) -> Response:
        users = [
            use_quest.user
            for use_quest in UseQuests.objects.filter(quest=quest)
    ...
    @classmethod
    def back_tg_quest(
        cls, user: Users, quest: Quests, use_quest: UseQuests
    ) -> Response:
...
class Quest_MA_Methods:
    ...
    def success_idea_daily(
        cls, user: Users, quest: Quests, use_quest_obj: UseQuests
    ) -> Response:
        # NOTE создаем связь на случай если она не была создана, но такого не может быть
        attempt = (
            QuestModerationAttempt.objects.filter(
                use_quest=use_quest_obj, moderation_status="pending"
            )
            .order_by("id")
            .first()
        )
        ...
        # Расчет награды
        if quest.type_quest == "daily" and quest.quest_data.scale_type == "x_count_use":
        ...
        raise RaisesResponse(data=add_starcoins, status=status.HTTP_200_OK)

    @classmethod
    def delete_idea(cls, quest: Quests, use_quest_obj: UseQuests) -> Response:
        attempt = (
            QuestModerationAttempt.objects.filter(
                use_quest=use_quest_obj, moderation_status="pending"
            )
        ...
        if quest.type_quest in ["idea", "daily"]:
            ...
            if use_quest_obj.count_use:
                if quest.type_quest == "daily":
                    use_quest_obj.updated_at = use_quest_obj.updated_at - timedelta(
                        days=1
                    )
    ...
    @classmethod
    def delete_old_quest(cls) -> Response:
        ...
        old_attempts = QuestModerationAttempt.objects.filter(
            moderation_status="pending", created_at__lte=cutoff_time
        ).all()

        data = []
        for attempt in old_attempts:
            try:
                if "SMM" in attempt.use_quest.quest.quest_data.title:
                    continue
                ...
                if attempt.use_quest.count_use:
                    if attempt.use_quest.quest.type_quest == "daily":
                        attempt.use_quest.updated_at = (
                            attempt.use_quest.updated_at - timedelta(days=1)
                        )
                ...
                logger.exception(
                    f"Error auto-rejecting attempt {attempt.id}: {e.__class__.__name__} {e}"
                )
                continue

        raise RaisesResponse(data=data, status=status.HTTP_200_OK)
