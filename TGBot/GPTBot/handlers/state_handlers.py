...
async def handle_text_prompt(message: types.Message, state: FSMContext):
    ...
    if is_timeout(user_id):
        await message.reply("<b>⏳ Пожалуйста, подождите 20 секунд перед следующим запросом.</b>")
        return
    ...
    try:
        ...
        await Func.send_error_to_developer(f"{user_id}\n<b>Ошибка генерации: {e.__class__.__name__} -> {e}</b>")
