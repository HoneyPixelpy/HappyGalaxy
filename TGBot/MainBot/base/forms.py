...
class Sigma_BoostsForms:
    ...
    async def upgrade(
        ...
        """
        try:
            upgrade_data = await Sigma_BoostsMethods().upgrade_boost(
                user, 
                name
                )
            if not upgrade_data:
...
class Lumberjack_GameForms:
    ...
    async def click_cell(
        ...
        """
        if isinstance(game_user.current_grid[row][col], int) and game_user.current_grid[row][col] == 1:
            return await Lumberjack_GameMethods().process_click(
                user.user_id,
                self.energy_in_click,
                row, 
                col
                )
...
class UsersForms:
    ...
    async def registration(
        ...
        """
        return await UserMethods().complete_registration(
            user,
            state_data
        )
