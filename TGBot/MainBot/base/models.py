...
class RewardData(BaseModel):
    starcoins_for_referal: Optional[int] = Field(None, description="Награда приглашенному в бота")
    starcoins_for_referer: Optional[int] = Field(None, description="Награда пригласившеми в бота")
    starcoins_parent_bonus: Optional[int] = Field(None, description="Награда за вступление в семью")
...
class Purchases(PydanticBase):
    ...
    def convert_to_utc_for_storage(cls, value: Optional[datetime]) -> Optional[datetime]:
        if value is None:
            return None
...
class Pikmi_Shop(PydanticBase):
    ...
    def round_price(cls, value) -> Optional[float]:
        ...
        try:
            ...
            return int(value_float) if round(value_float, 4) == int(value_float) else round(value_float, 4)
...
class Sigma_Boosts(PydanticBase):
    ...
    income_level: int = Field(0, ge=0, le=19, description="Уровень дохода за клик (0-19)")
    energy_capacity_level: int = Field(0, ge=0, le=7, description="Уровень запаса энергии (0-7)")
    recovery_level: int = Field(0, ge=0, le=2, description="Скорость восстановления (0-2)")
    passive_income_level: int = Field(0, ge=0, le=3, description="Пассивный доход (0-3)")
...
class Lumberjack_Game(PydanticBase):
    id: Optional[int] = Field(None, description="ID в базе данных")
    user: Optional[Users] = Field(None, description="Объект пользователя")
    game_date: datetime = Field(default_factory=datetime.now, description="Дата создания игры")
    ...
    current_grid: List[Any] = Field(default_factory=list, description="Текущее состояние поля 4x5")
...
class GeoHunter(PydanticBase):
    ...
    total_false: int = Field(0, ge=0, description="Общее количество не правильных ответов")
    total_currency: float = Field(0.0, description="Заработанная валюта")
...
class StatusType(str, Enum):
    ...
    def items(cls) -> Dict[str,str]:
        return {value.value: texts.InteractiveGame.Status.__dict__[value.value] for value in cls.__members__.values()}
...
class GameType(str, Enum):
    ...
    @classmethod
    def items(cls) -> Dict[str,str]:
        return {value.value: texts.InteractiveGame.Btns.__dict__[value.value] for value in cls.__members__.values()}
...
class InteractiveGameBase(BaseModel):
    ...
    reward_type: Optional[RewardType] = Field(None, description="Способ распределения вознаграждения")
    ...
    min_players: Optional[int] = Field(None, description="Минимальное количество игроков")
    max_players: Optional[int] = Field(None, description="Максимальное количество игроков")
    
    type_game: Optional[GameType] = Field(None, description="Тип игры")
    
    game_status: Optional[StatusType] = Field(None, description="Статус")
    
    start_invite_at: Optional[datetime] = Field(None, description="Время начала приглашения")
...
class InteractiveGameAllInfo(BaseModel):
    id: Optional[int] = Field(None, description="ID в базе данных")
    game: Optional[InteractiveGameBase] = Field(None, description="Игра")
    game_datas: Optional[List[InteractiveGameData]] = Field(None, description="Данные Играков")
