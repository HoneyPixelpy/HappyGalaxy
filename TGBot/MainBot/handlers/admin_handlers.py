...
from MainBot.base.orm_requests import QuestsMethods

from MainBot.handlers.message_handlers import text_over
from MainBot.utils.Forms import BonusBoosters, PersonalForms, Profile
...
from MainBot.filters.chat_types import IsAdmin, ChatTypeFilter
...
# @admin_router.message(Command(commands="add_channel_quest"), StateFilter(None))
# async def add_channel_quest(message: types.Message):
# 	channel_data: list[str] = message.text.replace("/add_channel_quest", "").strip().split("|")

# 	try:
# 		await QuestsMethods().create_quest(
# 			title=channel_data[0], # title
# 			description=channel_data[1], # descrition
# 			url=channel_data[2], # link
# 			chat_id_name=channel_data[3], # chat_id_name
# 			reward_starcoins=float(channel_data[4]),  # starcoins
# 			type=channel_data[5],  # type
# 			min_rang_level=channel_data[6],  # min_rang_level
# 			max_rang_level=channel_data[7],  # max_rang_level
# 			role=channel_data[8],  # role
# 			type_quest='subscribe'
# 		)
# 	except Exception as ex:
# 		logger.exception(f"{ex.__class__.__name__} {ex}")
# 		await Func.send_error_to_developer(f"{ex.__class__.__name__} {ex}")
# 		await message.answer(
# 			texts.Personal.Error.undefined_error
# 		)

# @admin_router.message(Command(commands="add_idea_quest"), StateFilter(None))
# async def add_idea_quest(message: types.Message):
# 	channel_data: list[str] = message.text.replace("/add_idea_quest", "").strip().split("|")

# 	try:
# 		await QuestsMethods().create_quest(
# 			title=channel_data[0], # title
# 			description=channel_data[1], # descrition
# 			call_action=channel_data[2], # call_action
# 			content=channel_data[3], # content
# 			count_use=channel_data[4], # count_use
# 			reward_starcoins=float(channel_data[5]),  # reward_starcoins
# 			type=channel_data[6],  # type
# 			min_rang_level=channel_data[7],  # min_rang_level
# 			max_rang_level=channel_data[8],  # max_rang_level
# 			role=channel_data[9],  # role
# 			success_admin=channel_data[10],  # success_admin
# 			type_quest='idea'
# 		)
# 	except Exception as ex:
# 		logger.exception(f"{ex.__class__.__name__} {ex}")
...
# @admin_router.message(Command(commands="add_daily_quest"), StateFilter(None))
# async def add_daily_quest(message: types.Message):
# 	channel_data: list[str] = message.text.replace("/add_daily_quest", "").strip().split("|")
...
# 			count_use=channel_data[4], # count_use
# 			reward_starcoins=float(channel_data[5]),  # reward_starcoins
# 			scale_type=channel_data[6],  # scale_type
# 			min_rang_level=channel_data[7],  # min_rang_level
# 			max_rang_level=channel_data[8],  # max_rang_level
# 			type=channel_data[9], # type
# 			role=channel_data[10], # role
# 			success_admin=channel_data[11], # success_admin
# 			type_quest='daily'
# 		)
# 	except Exception as ex:
# 		logger.exception(f"{ex.__class__.__name__} {ex}")
...
@admin_router.message(StateFilter(None), F.content_type.in_({'animation', 'audio', 'document', 'photo', 'sticker', 'video', 'video_note', 'voice', 'contact', 'location', 'venue', 'poll', 'dice', 'game', 'emoji'}))
...
async def find_user_id(message: types.Message, state: FSMContext):
	...
	if message.text == str(texts.Admin.Btns.back_to_admin):
		await message.answer(texts.Admin.Texts.main, reply_markup = await reply.amdin_markup())
		await state.clear()
		return
	try:
...
async def find_send_msg(message: types.Message, state: FSMContext):
	...
	try:
		await message.bot.send_message(
			chat_id=find_user_id,
			text=message.html_text
		)
		await message.answer(str(texts.Admin.FindUser.message_send))
	except Exception as ex:
		logger.exception(str(ex))
		await message.answer(str(texts.Error.Notif.no_send_msg).format(str(ex.__class__.__name__)))
...
async def find_change_balance(message: types.Message, state: FSMContext):
	...
	answer_text = await Profile().change_user_balance(
		find_user_id,
		new_balance
	)
...
async def find_purchases(call: types.CallbackQuery):
	...
	for task in await Profile().get_purchases_by_user(
		find_user_id
		):
...
async def delete_purchas(call: types.CallbackQuery):
	...
	await Profile().delete_output_task(
		call,
		target_user_id,
		task_id,
		bool_send_msg
	...
	await call.message.bot.send_message(
		...
		text=str(texts.Admin.Texts.task_delete)
	)
...
async def delete_purchas(call: types.CallbackQuery):
	...
	await Profile().rollback_registration(
		call,
		target_user_id
	)
...
async def Mailing_Pin(message: types.Message, state: FSMContext):
	if message.text == str(texts.Admin.Btns.back_to_admin):
		await message.answer(
			texts.Admin.Texts.main, 
			reply_markup = await reply.amdin_markup()
			)
		await state.clear()
		return
...
async def Mailing_Media(message: types.Message, state: FSMContext):
	if message.text == texts.Btns.back:
		await message.answer(str(texts.Admin.Mailing.step_one), reply_markup = await reply.Pin_Mailing_markup())
		...
		message_now = await message.bot.send_message(chat_id=message.from_user.id,text=str(texts.Admin.Mailing.step_three), reply_markup = await reply.back())
		await state.update_data(
			message_id=message_now.message_id,
...
async def Mailing_Text(message: types.Message, state: FSMContext, user: Users):
	if message.text == texts.Btns.back:
		await message.answer(str(texts.Admin.Mailing.step_two), reply_markup = await reply.back_to_amdin_markup(no_media_btn=True))
		await state.set_state(Mailing.Media)
	else:
...
async def Mailing_Confirm(message: types.Message, state: FSMContext):
	if message.text == texts.Btns.back:
		await message.answer(str(texts.Admin.Mailing.step_three), reply_markup = await reply.back())
		await state.set_state(Mailing.Text)
...
async def CreateBonus_type_bonus(message: types.Message, state: FSMContext, user: Users):
...
async def CreateBonus_size_starcoins(message: types.Message, state: FSMContext, user: Users):
	if message.text == texts.Btns.back:
		...
		await BonusBoosters().max_quantity_bonus(
			...
			user, 
			state,
			{"size_starcoins": int(message.text)}
		)
	else:
		await message.answer(str(texts.Error.Notif.get_number))
		return
...
async def CreateBonus_max_quantity(message: types.Message, state: FSMContext, user: Users):
	if message.text == texts.Btns.back:
		await BonusBoosters().size_starcoins_bonus(message, user, state)
		return
	elif message.text.isdigit():
...
async def CreateBonus_click_scale(message: types.Message, state: FSMContext, user: Users):
	if message.text == texts.Btns.back:
		await BonusBoosters().type_bonus(message, user, state)
		return
	else:
		try:
			await BonusBoosters().expires_at_hours_bonus(
				...
				user, 
				state,
				{"click_scale": float(message.text)}
			)
		except ValueError:
			await message.answer(str(texts.Error.Notif.get_number))
			return
...
async def CreateBonus_expires_at_hours(message: types.Message, state: FSMContext, user: Users):
	if message.text == texts.Btns.back:
		await BonusBoosters().type_bonus(message, user, state)
		return
	else:
...
async def text_admin(message: types.Message, state: FSMContext, user: Users):
	...
	if message.text.lower() in ["admin","/admin","админ","админка"]:
		...
		message_now = await message.bot.send_message(
			chat_id=message.from_user.id,text=str(texts.Admin.Mailing.step_one), 
