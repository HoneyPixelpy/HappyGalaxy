...
from MainBot.utils.Forms import FamilyTies, Authorisation, \
    BonusBoosters, Boosts, PersonalForms, Profile, Quests, Shop
from MainBot.utils.Games import LumberjackGame, LumberjackManager, GeoHunt, GeoHuntManager
...
async def why_are_you(call: types.CallbackQuery, state: FSMContext, user: Users):
    ...
    await Authorisation.why_are_you(
        call.message.bot,
        state,
        user
    )
...
@callback_router.callback_query(F.data.startswith("sure_role|"), Auth_state.why_are_you) # , StateFilter("*")
...
async def gender(call: types.CallbackQuery, state: FSMContext, user: Users):
    ...
    if call_data == "back":
        ...
        await Authorisation.age_participant(
            call,
            state,
            user
        )
...
async def gender(call: types.CallbackQuery, state: FSMContext, user: Users):
    await PersonalForms().age_worker(
        call,
        state,
        user
    )
...
# @callback_router.callback_query(F.data.startswith("success_fio|"), Auth_state.Success_FIO)
# async def success_fio(call: types.CallbackQuery, state: FSMContext, user: Users):
#     call_data = call.data.split("|")
#     if call_data[1] == "yes":
...
#                 state_data["role"]
#             )
#         # elif state_data["role"] in ["parent", "worker"]:
...
# @callback_router.callback_query(F.data.startswith("success_fio|"), RegPersonal.Success_FIO)
# async def success_fio(call: types.CallbackQuery, state: FSMContext, user: Users):
#     call_data = call.data.split("|")
#     if call_data[1] == "yes":
#         # state_data = await state.get_data()
...
#             )
#     elif call_data[1] == "other":
#         await PersonalForms().back_fio_worker(
#             call.message,
#             state,
#             user
#         )
...
async def data_validation(call: types.CallbackQuery, state: FSMContext, user: Users):
    call_data = call.data.split("|")
    if call_data[1] == "go":
        await Authorisation.success_ruls(
            call,
            state,
            user
        )
    elif call_data[1] == "edit":
        await Authorisation.back_gender_participant(
            call.message,
            state
        )

@callback_router.callback_query(F.data.startswith("data_validation|"), RegPersonal.final)
async def data_validation(call: types.CallbackQuery, state: FSMContext, user: Users):
    call_data = call.data.split("|")
    if call_data[1] == "go":
        await PersonalForms().success_ruls(
            call,
            state,
            user
        )
    elif call_data[1] == "edit":
        await PersonalForms().back_gender_worker(
            call.message,
            state
        )
...
async def game(call: types.CallbackQuery, user: Users):
    ...
    if (
        ...
        not await FamilyTies().info_parent(user, call.message)
        ):
        return
        ...
        if user.all_starcoins >= entry_threshold_geo_hunt:
            ...
            await call.message.answer(
                text=texts.Game.GeoHunt.young_to_geohunter.format(
                    bot_name=bot_name,
                    msg_id=call.message.message_id + 1
                ),
...
async def get_product(call: types.CallbackQuery, user: Users):
    ...
    await Shop().get_product(call, user, product_id)

@callback_router.callback_query(F.data.startswith("buy_product|"))
async def buy_product(call: types.CallbackQuery, user: Users):
    ...
    if product_id == "back":
        ...
        if status:
            await Shop().buy_product(
                call,
                user,
                error_msg_or_product
            )
        else:
            try:
                await call.answer(
                    text=error_msg_or_product,
                    show_alert=True
                )
            except:
...
async def check_sub(call: types.CallbackQuery, state: FSMContext, user: Users):
    ...
    if status:
        await Quests().success_subscribe_quest(
            call,
            user,
            error_msg_or_quest
        )
...
async def verif_interactive_game(call: types.CallbackQuery, user: Users):
    if call.data.split("|")[1] == "success":
        await InteractiveGame().success_create_game(
            user, 
            call,
            call.data.split("|")[2]
            )
    elif call.data.split("|")[1] == "delete":
        await InteractiveGame().delete_create_game(
            user, 
            call,
            call.data.split("|")[2]
            )
        ...
        await call.answer(
            "Error: verif_interactive_game",
            show_alert=True
            )

@callback_router.callback_query(F.data.startswith("refresh_interactive_game|"))
async def refresh_interactive_game(call: types.CallbackQuery, user: Users, state: FSMContext):
    game_id = int(call.data.split("|")[1])
    await InteractiveGame().refresh_ready_info(
        user,
        call,
        game_id
        )
...
async def login_interactive_game(call: types.CallbackQuery, user: Users, state: FSMContext):
    game_id = int(call.data.split("|")[1])
    await InteractiveGame().invite_game(
        user,
        call,
        game_id
        )
...
async def delete_interactive_game(call: types.CallbackQuery, user: Users, state: FSMContext):
    game_id = int(call.data.split("|")[1])
    await InteractiveGame().delete_game(
        user,
        call,
        game_id
        )
...
async def refresh_info_game(call: types.CallbackQuery, user: Users, state: FSMContext):
    game_id = int(call.data.split("|")[1])
    await InteractiveGame().refresh_info(
        user,
        call,
        game_id
        )
...
async def start_interactive_game(call: types.CallbackQuery, user: Users, state: FSMContext):
    game_id = int(call.data.split("|")[1])
    await InteractiveGame().start_game(
        user,
        call,
        game_id
        )
...
async def players_end_game(call: types.CallbackQuery, user: Users, state: FSMContext):
    ...
    await InteractiveGame().finally_game(
        user,
        call,
        game_id,
        pagination
...
async def end_game(call: types.CallbackQuery, user: Users, state: FSMContext):
    game_id = int(call.data.split("|")[1])
    await InteractiveGame().end_game(
        user,
        call,
        game_id
        )
