...
from MainBot.utils.Forms import FamilyTies, Season, \
    PersonalForms, Profile, Shop, Quests, GiveLinksForm
from MainBot.utils.Games import LumberjackGame, LumberjackManager, GeoHuntManager
import texts
...
async def cmd_start(message: types.Message, user: Users):
    ...
    if url_data:
        ...
        elif await GiveLinksForm().activate(
                message,
                user,
                url_data
            ):
...
async def profile(message: types.Message, state: FSMContext, user: Users):
    try:
        ...
        if key and await PersonalForms().check_key(key):
            await PersonalForms().gender_worker(
                user,
                message, 
                state, 
                key
