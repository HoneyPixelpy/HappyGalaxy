...
from MainBot.state.state import Auth_state, Daily, Idea, Msg_Delete, Offer, RegPersonal, SInteractiveGame, SPromocodes, VKProfile
from MainBot.utils.Forms import Authorisation, PersonalForms, \
    Profile, Quests, Shop, Promocodes
...
async def Auth_state_why_are_you(message: types.Message, state: FSMContext, user: Users):
    await message.delete()
...
async def Auth_state_Age(message: types.Message, state: FSMContext, user: Users):
    if message.text == texts.Btns.back:
        ...
        status, error_msg_or_age = await Authorisation.check_age(
            message.text,
            state
            )
        if status:
            await Authorisation.fio_participant(
                message,
                state,
                user,
                error_msg_or_age
...
async def RegPersonal_Age(message: types.Message, state: FSMContext, user: Users):
    if message.text == texts.Btns.back:
        ...
        if status:
            await PersonalForms().fio_worker(
                message,
                state,
                user,
                error_msg_or_age
...
async def Auth_state_FIO(message: types.Message, state: FSMContext, user: Users):
    ...
    if message.text == texts.Btns.back:
        await Authorisation.back_age_participant(
            message,
            state,
            user
        )
...
async def RegPersonal_FIO(message: types.Message, state: FSMContext, user: Users):
    if message.text == texts.Btns.back:
        await PersonalForms().back_age_worker(
            message,
            state,
            user
        )
...
async def Offer_shop(message: types.Message, state: FSMContext, user: Users):
    if message.text == texts.Btns.back:
        ...
        await Shop().send_offer(
            message,
            state,
            user
        )

@state_router.message(Auth_state.Gender,Auth_state.Success_FIO,Auth_state.success_ruls,Auth_state.final)
async def click_inline_kb(message: types.Message):
    await message.answer(
        texts.Error.Notif.no_btn + texts.Error.Notif.as_needed
    )
    await message.delete()
...
async def VKProfile_url(message: types.Message, state: FSMContext, user: Users):
    if message.text == texts.Btns.back:
        ...
        text, keyboard = await Quests().verif_vk_profile_link(
            state,
            user,
            message.text
        )
...
async def Idea_wait(message: types.Message, state: FSMContext, user: Users):
    ...
    if message.text == texts.Btns.back:
        await state.clear()
        await Quests().get_idea_quest(
            message, 
            user, 
            data['id']
            )
    else:
        await Quests().check_idea_quest(
            message,
            state,
            user
        )
...
async def Daily_wait(message: types.Message, state: FSMContext, user: Users):
    ...
    if message.text == texts.Btns.back:
        ...
        await Quests().check_daily_quest(
            message,
            state,
            user
        )
...
async def SInteractiveGame_wait_data(message: types.Message, state: FSMContext, user: Users):
    await message.bot.send_message(
        chat_id=user.user_id,
        text=texts.Texts.start,
        reply_markup=await reply.main_menu(user)
