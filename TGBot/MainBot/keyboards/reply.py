...
class KB:
    ...
    async def nickname(
        ...
        if role != "child":
            keyboard.append([
                types.KeyboardButton(text=texts.Btns.select_later)
            ])
        keyboard.append([
            types.KeyboardButton(text=texts.Btns.back)
        ])
    ...
    async def start_wait(cls) -> types.ReplyKeyboardMarkup:
        return types.ReplyKeyboardMarkup(
                keyboard=[
                    [
                        types.KeyboardButton(text=texts.Start.Btns.ready)
                    ]
                ],
                resize_keyboard=True,
                one_time_keyboard=True
    ...
    async def main_menu(cls, user: Users) -> types.ReplyKeyboardMarkup:
        ...
        if (
            user.role_private in ["parent", "child"] or 
            ...
            keyboard.append([
                types.KeyboardButton(text=texts.Btns.game)
            ])

        keyboard.append([
            types.KeyboardButton(text=texts.Btns.quests)
        ])
        ...
        # ])
        keyboard.append([
            types.KeyboardButton(text=texts.Btns.help)
        ])
        return types.ReplyKeyboardMarkup(
                keyboard=keyboard,
                resize_keyboard=True,
    ...
    async def back_to_amdin_markup(
        ...
        *,
        ...
        row_buttons = []
        if del_channel_btn:
            row_buttons.append(types.KeyboardButton(
                text=texts.Admin.Btns.delete_channel)
                )
        if del_personal_btn:
            row_buttons.append(types.KeyboardButton(
                text=texts.Admin.Btns.delete_personal)
                )
        if no_media_btn:
            row_buttons.append(types.KeyboardButton(
                text=texts.Admin.Mailing.no_media)
                )
        ...
        if no_media_btn:
            keyboard_list.append([types.KeyboardButton(
                text=texts.Btns.back)]
                )
        else:
            keyboard_list.append([types.KeyboardButton(
                text=texts.Admin.Btns.back_to_admin)]
                )
