...
import texts
from MainBot.utils.Forms import FamilyTies, Authorisation, UTMLinksForm
...
class UserDataSession(BaseMiddleware):
    ...
    async def __call__(
        ...
        try:    logger.debug(event.event.chat.id)
        except: pass
        ...
        if not user:
            try:
                ...
                if "add_family_" in ref_str: ref_str = ref_str.replace("add_family_", "")
            except:
                ref_str = ""
    ...
    async def update_tg_username(
        ...
        if from_user.username and user.tg_username != from_user.username:
            await UserMethods().update_username(
                user,
                from_user.username
            )

    async def utm_activate(
        self,
        user: Users,
        url_data: str
        ) -> None:
    ...
    async def registration_user(
        ...
        if (
            referral_user_id and
            referral_user_id.isdigit() and 
            int(referral_user_id) != user_data.id
            ):
