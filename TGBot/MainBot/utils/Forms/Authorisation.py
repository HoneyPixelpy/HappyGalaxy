...
class Authorisation:
            ...
            if message_data["type"] == "sticker":
                try:
                    await bot.send_sticker(
                        chat_id=user_id,
                        sticker=message_data["message_key"]
                    )
    ...
    async def sure_role(
        ...
        await bot.send_message(
            chat_id=user.user_id,
            text=text,
            reply_markup=await inline.sure_role(role)
        )
    ...
    async def gender_participant(
        ...
        last_msg = await call.message.bot.send_message(
            chat_id=user.user_id,
            text=texts.Start.Texts.child_reg_msg_1
            )

        for msg_text in [
            texts.Start.Texts.child_reg_msg_3
            ]:
            last_msg = await call.message.bot.send_message(
                chat_id=user.user_id,
                text="...",
    ...
    async def fio_participant(
        ...
        await message.bot.send_message(
            chat_id=user.user_id,
            text=texts.Start.Texts.first_name if data["role"] == "child" else texts.Start.Texts.fio,
            reply_markup=await reply.back(
    ...
    async def check_fio(
        ...
        if any('--' in part or part.startswith('-') or part.endswith('-') for part in parts):
    ...
    async def create_nickname(
        ...
        await message.bot.send_message(
            ...
            reply_markup=await reply.nickname(
                role
            )
        )
    ...
    async def check_nickname(
        ...
        if any(bad_word in normalized_nick.lower() for bad_word in await cls.banned_words()):
    ...
    async def data_validation(
        ...
        await message.bot.send_message(
            ...
            text=texts.Start.Texts.data_validation.format(
                ...
                age=(datetime.now(pytz.timezone('Europe/Moscow')) - state_data["age"]).days // 365,
                title="Имя" if state_data["role"] == "child" else "ФИО",
                name=state_data["name"],
                supername=state_data["supername"],
                nickname=state_data["nickname"] if state_data["nickname"] else "Не указано",
    ...
    async def success_ruls(
        ...
        await call.message.bot.send_message(
            chat_id=user.user_id,
            text=texts.Start.Texts.wait_passport
        )
        await asyncio.sleep(4)
        
        await cls.final_regisration(
            call,
            state,
            user
        )
    ...
    async def final_regisration(
        ...
        await call.message.bot.send_message(
            chat_id=user.user_id,
            text=texts.Start.Texts.ready_passport
        )
