...
class Boosts:
    ...
    async def catalog(self, call: types.CallbackQuery, user: Users) -> None:
        """
        ...
        for boost_data in boosts_data:
            inline_keyboard.append(
                [
                    types.InlineKeyboardButton(
                        text=texts.Boosts.Btns.__dict__[boost_data["name"]].format(
                            now=boost_data["value"], max=boost_data["max_level"]
                        ),
            ...
            inline_keyboard.append(
                [
                    types.InlineKeyboardButton(
                        text=texts.Btns.back, callback_data="get_boost|back"
                    )
                ]
            )
    ...
    async def build_boost_text(
        ...
        return texts.Boosts.Texts.info.format(
            ...
            emoji=emoji,
            now_level=now_level,
            description=texts.Boosts.Descriptions.__dict__[name].format(
                next_level=max_level if now_level + 1 > max_level else now_level + 1
            ),
            upgrade_cost=price,
    ...
    async def get_boost(
        self, call: types.CallbackQuery, user: Users, name: str, alert: bool = False
    ) -> None:
        """
        ...
        if boost_info["boost_level"] < boost_info["max_level"]:
            ...
            inline_keyboard.append(
                [
                    types.InlineKeyboardButton(
                        text=texts.Btns.buy, callback_data=f"upgrade_boost|{name}"
                    )
                ]
            )
        else:
        ...
        inline_keyboard.append(
            [
                types.InlineKeyboardButton(
                    text=texts.Btns.back, callback_data="upgrade_boost|back"
                )
            ]
        )
    ...
    async def upgrade_boost(
        ...
        Отправляем сообщение с информацией о конкретном бусте
        """
        upgrade_data: dict = await Sigma_BoostsForms().upgrade(user=user, name=name)
        if not debug:
            if not upgrade_data:
                try:
                    await call.answer(
        ...
        if upgrade_data:
            ...
            try:
                await call.answer(
                    text=texts.Boosts.Texts.success.format(
                        emoji=upgrade_data["emoji"], level=upgrade_data["level"]
                    ),
                    show_alert=True,
                )
            except: # exceptions.TelegramBadRequest
            ...
            await call.message.bot.send_message(
                chat_id=user.user_id, text=texts.Error.Notif.undefined_error
            )
