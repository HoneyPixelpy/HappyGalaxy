...
from typing import AsyncGenerator, List, Optional, Tuple, Any, Coroutine
...
from MainBot.base.models import Users, RewardData
import texts
from MainBot.keyboards.reply import KB as reply
from config import bot_name, photo_id
from config import log_chat


class Profile:

    async def new_user_in_log(
        self,
        bot: Bot,
        user: Users
        ) -> None:
        ...
        await bot.send_message(
            ...
            text=texts.Profile.Texts.log_info.format(
                ...
                referal_id=user.referral_user_id if user.referral_user_id else "Без реферала",
                role=user.role
            ),
    ...
    async def find_user(
        ...
        inline_keyboard_list = [
            ...
            [
                ...
                types.InlineKeyboardButton(
                    text=ban_text,
                    callback_data=f"ban_user|{target_user.user_id}"
                )
            ],
            [
                types.InlineKeyboardButton(
        ...
        text = str(texts.Admin.Texts.info).format(
            ...
            last_name=escape(target_user.tg_last_name) if target_user.tg_last_name else None,
            role=target_user.role,
            nickname=target_user.nickname,
            title="Имя" if target_user.role_private == "child" else "ФИО",
            supername=target_user.supername,
            name=target_user.name,
            gender=await Func.gender_name(target_user.gender, str(target_user.role_private)),
            age=(datetime.now(pytz.timezone('Europe/Moscow')) - target_user.age).days // 365,
            phone=target_user.phone,
            starcoins=target_user.starcoins,
            all_purchases=len(purchases),
            active_purchases=len([purchase for purchase in purchases if not purchase.completed]),
            ...
            authorised_at=await Func.format_date(target_user.authorised_at) if target_user.authorised_at else "Не авторизован",
    ...
    async def rollback_registration(
        ...
        try:
            ...
            return str(texts.Error.Notif.no_change_balance).format(str(ex.__class__.__name__))
    ...
    async def change_user_balance(
        ...
        """
        try:
            new_starcoins = await UserMethods().update_balance(
                user_id,
                new_balance
            )
    ...
    async def get_purchases_by_user(
        ...
        if not purchases: return [(texts.Admin.Texts.no_tasks, None)]
            ...
            result.append(
                (
                    str(texts.Admin.FindUser.purchas).format(
                        ...
                        price=purchas.cost,
                        purchase_date=purchas.purchase_date.strftime("%d %m %Y") if purchas.purchase_date else "..."
                    ),
    ...
    async def get_all_output_tasks(
        self,
        ) -> AsyncGenerator[Tuple[Optional[str], Optional[types.InlineKeyboardMarkup]], None]:
        """
        Получаем все таски.
        """
        ...
        for purchas in purchases:
            ...
            if tasks_found: yield (texts.Admin.Texts.this_all_tasks,None)
            else:           yield (texts.Admin.Texts.all_no_tasks,None)
    ...
    async def notification_referal(
        ...
        """
        referal_user = await UserMethods().process_referral(
            user.user_id
            )
        if referal_user:
    ...
    async def notification_parent(
        ...
        message: types.Message,
        user: Users,
        parent_user: Optional[Users] = None
        ) -> Optional[bool]:
        """
        ...
        await message.bot.send_message(
            chat_id=user.user_id,
            text=texts.Family_Ties.Texts.success_parent
        )

        await message.bot.send_message(
            chat_id=parent_user.user_id,
            text=texts.Family_Ties.Texts.success_parent
        )
        return True
    ...
    async def mailing(
        ...
        **kwargs
        ...
        await message.answer("Рассылка запущена", reply_markup = await reply.amdin_markup())
