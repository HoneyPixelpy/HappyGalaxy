...
class FlagProcessor:
    ...
    def _get_country_info_cached(self, country_code: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    async def get_country_info(self, country_code: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    async def get_localized_name(self, english_name: str) -> Optional[str]:
        ...
        try:
            ...
            if len(translated) <= 14 and any('\u0400' <= c <= '\u04FF' for c in translated):
        ...
        for index, step_flag_path in enumerate(await self.get_step_flags(),start=1):
            ...
            if index == true_var:
                ...
                await redis_client.set(
                    ...
                    json.dumps({"id": step_flag_path['id'], "title": step_flag_path['title']}, ensure_ascii=False),
                    ex=60
                )
...
class Build(Flag):
        ...
        for data in datas:
            btns.append(
                types.InlineKeyboardButton(
                    text=data['title'],
                    callback_data="geo_hunt_click|{}".format(
                        data["id"]
                    )
                )
            )
    ...
    async def send(
        ...
        try:
            try:
                if new_msg:
                    raise Exception("Хотим сразу отправить сообщение без попытки изменить его")
...
class GeoHuntMain(Build):
    ...
    async def get_field(
        ...
        data, photo_id, true_emoji = await super().get_flag_data(
            user,
            redis_client
        )
        text = await super().text(
            user.starcoins,
            geo_hunter.current_energy,
            true_emoji
        )        
    ...
    async def handle_click(
        ...
        if user_choice:
            ...
            try:
                await call.answer(
                    texts.Game.GeoHunt.no.format(
                        name=true_var.get("title")
                    )
                )
            except:
                pass
...
class GeoHuntMonitoring(GeoHuntMain):
    ...
    async def _game_session_timeout(
        ...
        try:
            ...
            if session_data:
                timeout_counter = int(session_data.get(b'timeout_counter', b'0').decode()) + 1
                
                await bot.delete_message(
                    chat_id=user_id,
                    message_id=message_id
                )
                await bot.send_message(chat_id=user_id, text=texts.Game.GeoHunt.time_over)
    ...
    async def get_field(
        ...
        geo_hunter: Optional[GeoHunter] = None, # TODO должен быть обновленным после вычета энергии
        new_msg: bool = False,
        ...
        message_id = await super().get_field(call, user, redis_client, geo_hunter, new_msg)
        
        # Создаём задачу с таймаутом
        task = asyncio.create_task(
            self._game_session_timeout(
                call,
                user,
                geo_hunter,
                message_id
        ...
        expiry_time = datetime.datetime.now() + datetime.timedelta(seconds=self.GEO_HUNT_TIMEOUT)
        ...
        await redis_client.expire(f"geo_hunt_session:{user_id}", self.GEO_HUNT_TIMEOUT + 10)
    ...
    async def handle_click(
        ...
        await self.get_field(
            call, 
            game_user.user if game_user else user, 
            game_user
            )
...
class LockManager(GeoHuntMonitoring):
    ...
    async def speed_lock(self, call: types.CallbackQuery, user: Users) -> Optional[bool]:
        user_id = user.user_id
        redis_client = await self.redis.get_redis()
        ...
        try:
            async with self.user_lock(user_id):
                ...
                if last_click_str:
                    last_click = datetime.datetime.fromisoformat(last_click_str.decode())
