...
from MainBot.base.models import GameType, InteractiveGameAllInfo, InteractiveGameInfo, RewardType, Users, InteractiveGameBase, InteractiveGameData
...
class SupportInteractiveGame:
    ...
    async def check_key(
        ...
        return key in ["clear", "delete"] + list(InteractiveGameBase.model_fields.keys())
...
class InteractiveGameStorage:
    ...
    async def set(
        ...
        async with lock:
            if not self._redis: self._redis = await RedisManager().get_redis()
...
class InteractiveGameDB:
    ...
    async def delete_rejected_game(
        self,
        game_id: int
        ) -> Optional[InteractiveGameBase]:
        return await InteractiveGameMethods().delete_rejected(game_id)

    async def delete_pending_game(
        self,
        game_id: int
        ) -> Optional[InteractiveGameInfo]:
...
class NotificationInteractiveGame(InteractiveGameStorage, SupportInteractiveGame):
    ...
    async def send_moderation(
        ...
        await call.bot.send_message(
            ...
            text=result_worker_game + texts.InteractiveGame.Create.user_info_game.format(
                role=user.role_name,
    ...
    async def send_delete(
        ...
        await call.bot.send_message(
            chat_id=user.user_id,
            text=texts.InteractiveGame.Create.delete_game.format(
                title=game.title
            )
        )
    ...
    async def send_success(
        ...
        await call.bot.send_message(
            chat_id=user.user_id,
            text=texts.InteractiveGame.Create.success_game.format(
                title=game.title
            )
        )

    async def send_no_active_users(
        self,
        user: Users,
        call: CallbackQuery
        ) -> None:
        await call.bot.send_message(
            chat_id=user.user_id,
            text=texts.InteractiveGame.Ready.no_active_users
        )
    ...
    async def send_new_try_start(
        ...
        await call.bot.send_message(
            ...
            reply_markup=await inline.new_try_interactive_game(
                game.id
                )
        )

    async def false_invite(
        self,
        call: CallbackQuery
        ) -> None:
        await call.answer(
            text=texts.InteractiveGame.Invite.false
        )
        await call.message.delete()
    ...
    async def true_invite(
        ...
        await call.answer(
            text=texts.InteractiveGame.Invite.true
        )
        await call.message.delete()
        ...
        await call.bot.send_message(
            ...
            text=texts.InteractiveGame.Invite.notif_admin.format(
                user=await Func.format_tg_info(
                    user.user_id, 
                    user.tg_username
                    ),
                title=game.title
    ...
    async def fail_delete_pending(
        self,
        call: CallbackQuery
        ) -> None:
        await call.answer(
            text=texts.InteractiveGame.Ready.fail_delete_pending
        )
    ...
    async def send_delete_pending(
        ...
        await bot.send_message(
            chat_id=user.user_id,
            text=texts.InteractiveGame.Invite.cancel.format(
                title=game.title
            )
        )

    async def send_error_activate_game(
        self,
        user: Users,
        bot: Bot
        ) -> None:
        await bot.send_message(
            chat_id=user.user_id,
            text=texts.InteractiveGame.Activate.error
        )
    ...
    async def send_game_info(
        ...
        *,
        ...
        text=texts.InteractiveGame.Activate.info.format(
            ...
            current_time=(datetime.now(pytz.timezone('Europe/Moscow')) - game.start_game_at).seconds // 60
        )
        keyboard=await inline.game_info(
            game.id,
            user.user_id == game.user.user_id
            )
        try:
            if not call: raise Exception("Надо просто отправить сообщение")
    ...
    async def send_error_finally_game(
        self,
        user: Users,
        bot: Bot
        ) -> None:
        await bot.send_message(
            chat_id=user.user_id,
            text=texts.InteractiveGame.Error._finally
        )

    async def send_error_end_game(
        self,
        user: Users,
        bot: Bot
        ) -> None:
        await bot.send_message(
            chat_id=user.user_id,
            text=texts.InteractiveGame.End.error
        )
    ...
    async def send_result_game_creator(
        ...
        text = texts.InteractiveGame.End.creator.format(
            ...
            count_winers=len([game_data for game_data in game_all_info.game_datas if game_data.result == "win"]),
            calc_starcoins=sum([game_data.reward_starcoins for game_data in game_all_info.game_datas if game_data.result == "win"]),
            count_losers=len([game_data for game_data in game_all_info.game_datas if game_data.result == "lose"])
        )
        try:
            await call.message.edit_text(
                text=text
    ...
    async def send_result_game_win(
        ...
        await bot.send_message(
            ...
            text=texts.InteractiveGame.End.win.format(
                title=game.title,
                reward_starcoins=game_data.reward_starcoins
            )
        )
    ...
    async def send_result_game_lose(
        ...
        await bot.send_message(
            chat_id=user.user_id,
            text=texts.InteractiveGame.End.lose.format(
                title=game.title
            )
        )
    ...
    async def send_result_game_draw(
        ...
        await bot.send_message(
            chat_id=user.user_id,
            text=texts.InteractiveGame.End.draw.format(
                title=game.title
            )
        )
...
class CreateInteractiveGame(MainInteractiveGame):
    ...
    async def create_info_msg(
        ...
        *,
        ...
        if call:
            await call.message.edit_text(
                text,
                reply_markup=await inline.create_game(await self.check_required_data(data))
            )
        else:
    ...
    async def edit_state(
        ...
        if key == "title":
            ...
            if len(value) > 1024:
                ...
                return
        elif key in ["reward_starcoins", "min_rang", "max_rang", "min_players", "max_players"]:
            try:
                float(value)
            except ValueError:
                await message.answer(
    ...
    async def create_game(
        ...
        if await super().check_required_data(data):
            if data.get('message_id'): del data['message_id']
...
class ReadyInteractiveGame(MainInteractiveGame):
    ...
    async def invite_mailing(
        ...
        for active_user in active_users:
            ...
            if not await super().get(user, prymary_key=f"mailing_new_game:{game.id}"): break
            try:
                await mailing_fun(active_user)
                received += 1
                await super().set(user, "received", received, f"mailing_new_game:{game.id}")
            ...
            await bot.send_message(
                chat_id=user.user_id, 
                text="💡 <b>ВСЕ ПРИГЛАШЕНИЯ БЫЛИ РАЗОСЛАНЫ</b>\n"
                )
            await super().set(user, "received", f"Завершена на {received} пользователей", f"mailing_new_game:{game.id}")
    ...
    async def progress_text(
        ...
        return texts.InteractiveGame.Ready.ready_game.format(
            type_game=game.type_game,
            title=game.title,
            progress=received if isinstance(received, str) else f"{received}/{all_users}",
            _in=in_users,
    ...
    async def refresh_ready_info(
        ...
        user: Users,
        call: CallbackQuery,
        game: Union[InteractiveGameBase, int]
        ) -> None:
        """
    ...
    async def delete_game_mailing(
        ...
        for user in users:
            try:
                await super().send_delete_pending(
                    user, 
                    bot, 
                    game
                )
    ...
    async def delete_pending(
        ...
        await self.delete_game_mailing(
            call.bot, 
            game_info.game, 
            game_info.users
            )
...
class StartInteractiveGame(MainInteractiveGame):
    ...
    async def mailing_start_game(
        ...
        for user in users:
            if not await super().get(user, prymary_key=f"mailing_start_game:{game.id}"): break
            try:
...
class PlayersPanel(MainInteractiveGame):
        ...
        for index, user in enumerate(users):
            if index >= sp and index < lp:
                text += texts.InteractiveGame.Players.user.format(
                    emoji=texts.InteractiveGame.Players.Emoji.cgreen if str(user.user_id) in selected else texts.InteractiveGame.Players.Emoji.cgrey,
    ...
    async def build_reply_markup(
        ...
        # NOTE Протестировать на нескольких юзерах
        inline_keyboard = []
        data = await self.markup_data(game_info.users, game_info.game, callback_data, pagination)
        inline_keyboard.extend(
            [
                [
                    types.InlineKeyboardButton(
                        text=texts.InteractiveGame.Btns.calculate_victory if selected else "✖️",
                        callback_data=f"end_game|{game_info.game.id}" if selected else "null"
                    )
                ]
            ]
        )
    ...
    async def players_info(
        ...
        selected = await super().get(
            user,
            "selected",
            prymary_key=f"s_player_end_game|{game_info.game.id}"
        )
...
class FinallyInteractiveGame(PlayersPanel):
    ...
    async def finally_game(
        ...
        if user_id:
            ...
            if user_id == 'clear':
                ...
                selected.extend(
                    [
                        str(u.user_id) for u in game_info.users
                    ]
                )
            ...
            await super().set(user, "selected", selected, f"s_player_end_game|{game_info.game.id}")

        await super().players_info(
            user,
            call.bot,
            game_info,
            pagination,
    ...
    async def result_game(
        ...
        """
        bot = call.bot
        await super().send_result_game_creator(
            user, 
            bot,
            call, 
            game_all_info
            )
        await super().send_result_game_logs_chat(
            bot, 
            game_all_info
            )
...
class InteractiveGame(
    ...
    async def ready_info(
        ...
        await super().set(user, "all_users", len(active_users), f"mailing_new_game:{game.id}")
