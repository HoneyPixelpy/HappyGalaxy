...
async def load_config_from_forwards(*args, **kwargs) -> dict:
    async with file_lock:
        async with aiofiles.open(
            BASE_DIR / config_from_forwards, "r", encoding="utf-8"
        ) as f:
            data = await f.read()
...
class Func:
    ...
    @classmethod
    async def get_media_types(cls, form_name: str = "all") -> Set[str]:
        if form_name == "visible":
            return ("photo", "animation", "video", "document", "video_note", "sticker")
        else:
            return (
                "photo",
                "animation",
    ...
    @classmethod
    async def get_emoji_number(cls, number: int) -> str:
        result = ""
    ...
    async def format_tg_info(
        cls, uer_id: Union[str, int], user_name: Optional[str]
    ) -> str:
    ...
    @classmethod
    async def format_tg_names(cls, first_name: str, last_name: Optional[str]) -> str:
        text = first_name
        if last_name:
            text += f" {last_name}"
        return text
    ...
    async def format_nickname(
        cls, user_id: int, nickname: Optional[str], name: str, supername: Optional[str]
    ) -> str:
    ...
    @classmethod
    async def gender_name(cls, gender: str, role: str) -> str:
        if gender == "man":
    ...
    async def pin_msg_delete_notification(
        cls, bot: Bot, user_id: int, message_id: int, delete_msg_add: int = 1
    ) -> None:
        """
        Закрепляем сообщение.
        """
        await bot.pin_chat_message(
            chat_id=user_id, message_id=message_id, disable_notification=True
        )
                ...
                for part_text in text_parts:
                    if part_text or (part_text == text_parts[-1] and reply_markup):
                        mailing_message = await bot.send_message(
                            ...
                            reply_markup=(
                                reply_markup if part_text == text_parts[-1] else None
                            ),
                        )
                        delete_msg_add += 1
                if pin_bool:
    ...
    async def constructor_album_to_mailing_one_msg(
        ...
        for i, album_indice in enumerate(album_list_indices):
            text_parts = TextProcessor.prepare_text_parts(text[album_indice])
            if media_type[album_indice] == "photo":
                media_item = types.InputMediaPhoto(
                    media=media_content[album_indice], caption=text_parts.pop(0)
                )
    ...
    @classmethod
    async def send_error_to_developer(cls, text: str, bot: Bot = bot) -> None:
        tz = pytz.timezone("Europe/Moscow")
        text = f"{datetime.datetime.now(tz)}\n\n" + text
        await bot.send_message(chat_id=1894909159, text=text, parse_mode=None)

    @classmethod
    async def format_date(cls, data: datetime.datetime) -> str:
        RUSSIAN_MONTHS = (
            "января",
            "февраля",
            "марта",
    ...
    @classmethod
    def _can_form_album(
        cls, media_group_ids: List[Optional[int]]
