...
class TextProcessor:
    ...
    @classmethod
    def split_text(cls, text: str, max_length: int) -> Tuple[str, str]:
        """
        ...
        for match in matches:
            ...
            if is_closing:
                if tag_stack and tag_stack[-1] == tag_name:
                    ...
                    logger.warning(
                        f"Найден закрывающий тег '</{tag_name}>' без соответствующего открывающего в первой части текста."
                    )
            else:
        ...
        closing_tags = [
            cls._get_tag_string(tag_name, is_closing=True)
    ...
    def prepare_text_parts(
        cls, text: Optional[str] = None, first_len: Optional[int] = None
    ) -> List[str]:
        """
        ...
        first_part, remaining_text = cls.split_text(
            remaining_text, first_len if first_len else cls.MAX_CAPTION_LENGTH
        )
        parts.append(first_part)

        # Остальные части текста (для обычных сообщений) имеют другой лимит
        while remaining_text:
            part, remaining_text = cls.split_text(
                remaining_text, cls.MAX_MESSAGE_LENGTH
            )
            parts.append(part)
