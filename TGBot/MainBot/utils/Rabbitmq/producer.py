...
class RabbitMQProducer:
    ...
    async def track_action(
        ...
        if event.event_type == "callback_query":
            ...
            text, content_type, _, _ = await Func.parsing_message(message)
            event_data = {
                "user_id": from_user.id,
                "event_type": event.event_type,
                "text": text or None,
        ...
        self.produce_action(RabbitMQConfig.TOPICS.USER_ACTIONS, event_data)

    @logger.catch
    async def track_shop(self, user_id: int, product_id: int) -> None:
        event_data = {"user_id": user_id, "product_id": product_id}
        self.produce_action(RabbitMQConfig.TOPICS.SHOP_ACTIONS, event_data)

    @logger.catch
    async def track_quest(self, user_id: int, quest_id: int, action: str) -> None:
        event_data = {"user_id": user_id, "quest_id": quest_id, "action": action}
        self.produce_action(RabbitMQConfig.TOPICS.QUEST_ACTIONS, event_data)

    @logger.catch
    async def track_game(self, user_id: int, win_starcoins: int, game: str) -> None:
        event_data = {"user_id": user_id, "win_starcoins": win_starcoins, "game": game}
        self.produce_action(RabbitMQConfig.TOPICS.GAME_ACTIONS, event_data)
