...
class WaitingTask:
    ...
    async def pre_task_waiting(
        ...
        if wait_time > 0:
            logger.debug(f"Waiting for {wait_time} seconds until {target_time.strftime('%Y-%m-%d %H:%M:%S %Z%z')}")
            await asyncio.sleep(wait_time)
            logger.debug("Подождали!")
            return True
        else:
            logger.error("Заданное время находится в прошлом. Пожалуйста, установите будущее время.")
            return False
...
class Tasks:
    ...
    async def create_task_from_database_copy(self):
        while True:
            now, target_time = await WaitingTask.get_now_and_target_time_in_time_zone(hour=2)
