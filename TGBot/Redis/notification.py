...
async def send_rang_upgrade_message(
    ...
    try:
        ...
        await bot.send_message(
            chat_id=user_id, 
            text=text,
            reply_markup=await IKB.new_rang(new_quests)
            )
...
async def handle_continue_registration_mailing(*args, **kwargs) -> None:
    ...
    while True:
        try:
            # Проверяем Redis на наличие уведомлений
            continue_registration_json = await redis_client.lpop('bot:continue_registration')
            
            if continue_registration_json:
                continue_registration = json.loads(continue_registration_json)
                await continue_registration_mailing(
                    continue_registration['user_ids']
                )
            await asyncio.sleep(1)
    ...
    try:
        ...
        for user_id in user_ids:
            try:
                await bot.send_message(
                    chat_id=user_id,
                    text=texts.Profile.Texts.continue_registration
                )
                number += 1
                if number % 10 == 0:
                    logger.debug(f"Юзеров получило уже - {number} о предложении продолжить регистарцию")
    ...
    try:
        ...
        for mailing_data in mailing_datas:
            ...
            try:
                ...
                number += 1
                if number % 10 == 0:
                    logger.debug(f"Юзеров получило уже - {number} о просроченном квесте")
                await asyncio.sleep(0.3)
            except exceptions.TelegramRetryAfter as ex:
                logger.error(f"Сработало исключение!!!\n!!!{ex}")
                # await asyncio.sleep(ex.retry_after)
        ...
        logger.exception(f"Failed to send auto reject old quest attempts message to: {e}")
