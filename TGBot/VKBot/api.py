...
class VKGroup:
    ...
    @classmethod
    async def get_group_members(cls, group_id: int, group_token: str) -> list[int]:
    ...
    async def check_subscription(
        cls, user_id: int, group_id: int, group_token: str
    ) -> bool:
        try:
    ...
    @classmethod
    async def get_user_id(cls, username: str, group_token: str) -> Optional[int]:
        users = await API(token=group_token).users.get(user_ids=username)
        # logger.debug(users)
        return users[0].id if users else None

    @classmethod
    async def strip_url(cls, user_link: str) -> str:
    ...
    @classmethod
    async def check_invite(cls, vk_id: int, group_id: int, group_token: str) -> bool:
        return (
            False
            if not await cls.check_subscription(vk_id, group_id, group_token)
            else True
        )
